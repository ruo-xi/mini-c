...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef  Specifier ExtDecList FuncDec StructDec StructDecList CompSt VarList VarDec ParamDec Stmt ForDef StmList DefList Def DecList Dec Exp Args
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(EXT_VAR_DEF,$1,$2,NULL,yylineno);}   //该结点对应一个外部变量声明
         ...
         |Specifier ID LC StructDecList RC SEMI {$$=mknode(STRUCT_DEF,$4,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
         | error SEMI   {$$=NULL; }
         ;
Specifier:  TYPE    {$$=mknode(TYPE,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);if($1=="int")$$->type=INT;if($1=="float")$$->type=FLOAT;if($1=="char")$$->type=CHAR;if($1=="string")$$->type=STRING;if($1=="struct")$$->type=STRUCT;}
           ;
...
VarDec:  ID          {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        |ID LB INT RB  {$$=mknode(ARRAY,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);} // TODO
         ;
...
StructDecList: {$$=NULL;}
        | StructDec StructDecList {$$=mknode(STRUCT_DEC_LIST,$1,$2,NULL,yylineno);}
        ;
StructDec: VarList SEMI  {$$=mknode(STRUCT_DEC,$1,NULL,NULL,yylineno);}
        ;
...
ForDef: Exp SEMI Exp SEMI Exp {$$=mknode(FOR_DEF,$1,$3,$5,yylineno);}
       | SEMI SEMI {$$=NULL; }
       ;
